...
const EXPECTED_PREVIOUS_LATEST = "0024_initial_admin_bootstrap_state.sql";
const EXPECTED_CURRENT_LATEST = "0025_mutation_abuse_admission.sql";
...
try {
  ...
  const latestRelationBefore = await pool.query<{ relation: string | null }>(
    "SELECT to_regclass('public.mutation_rate_limit_buckets')::text AS relation",
  );
  ...
  const latestRelationAfter = await pool.query<{ bucket: string | null; charge: string | null }>(
    `SELECT
       to_regclass('public.mutation_rate_limit_buckets')::text AS bucket,
       to_regclass('public.mutation_rate_limit_charges')::text AS charge`,
  );
  if (
    latestRelationAfter.rows[0]?.bucket !== "mutation_rate_limit_buckets" ||
    latestRelationAfter.rows[0]?.charge !== "mutation_rate_limit_charges"
  ) {
