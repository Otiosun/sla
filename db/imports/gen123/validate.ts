...
if (DATABASE_URL === undefined)
  throw new Error("DATABASE_URL is required for Gen I-III validation");
...
export async function validateGen123(markValidated = true): Promise<Gen123ValidationReport> {
  ...
  try {
    const release = await pool.query<{ status: string }>(
    ...
    if (!new Set(["DRAFT", "VALIDATED"]).has(status))
      throw new Error(`Unexpected candidate status ${status}`);
    ...
    if (
      ...
      dexRow.min_dex !== 1 ||
      dexRow.max_dex !== 386 ||
      dexRow.distinct_slug !== 386
    )
      throw new Error(`National Dex/slug integrity failed: ${JSON.stringify(dexRow)}`);
    ...
    if ((invalidRanges.rows[0]?.total ?? 1) !== 0)
      throw new Error(`Invalid catalog ranges: ${invalidRanges.rows[0]?.total}`);
    ...
    if ((brokenRefs.rows[0]?.total ?? 1) !== 0)
      throw new Error(`Broken release references: ${brokenRefs.rows[0]?.total}`);

    const ruleset = await pool.query<{ id: string }>(
      "SELECT default_ruleset_id AS id FROM content_releases WHERE id=$1",
      [releaseId],
    );
    ...
    for (const nationalDex of [1, 25, 386]) {
      ...
      const actual = await pool.query<{
        ...
        `SELECT species_revision.catch_rate, species_revision.base_exp,
         ...
         WHERE species.national_dex=$2`,
        [releaseId, nationalDex],
      );
      const row = actual.rows[0];
      if (
        row === undefined ||
        row.catch_rate !== expected.captureRate ||
        row.base_exp !== expected.baseExperience ||
        [
          row.base_hp,
          row.base_attack,
          row.base_defense,
          ...
          row.base_sp_defense,
          row.base_speed,
        ].join(",") !== expected.stats.join(",")
      )
        throw new Error(`Species sample mismatch #${nationalDex}`);
    ...
    for (const sourceMoveId of [33, 53]) {
      ...
      const actual = await pool.query<{
        ...
        `SELECT revision.power, revision.accuracy, revision.max_pp FROM move_revisions revision JOIN moves move ON move.id=revision.move_id WHERE revision.content_release_id=$1 AND move.slug=$2`,
        [releaseId, expected.slug],
      );
      const row = actual.rows[0];
      if (
        row === undefined ||
        row.power !== expected.power ||
        row.accuracy !== expected.accuracy ||
        row.max_pp !== expected.pp
      )
        throw new Error(`Move sample mismatch ${expected.slug}`);
    ...
    const report: Gen123ValidationReport = {
      ok: true,
      releaseId,
      counts,
      coverage,
    ...
    if (markValidated && status === "DRAFT") {
      await pool.query(
        ...
        [
          ...
          hash({ source: GEN123_SOURCE, counts, coverage, samples }),
        ],
      );
    }
    return report;
...
if (process.argv[1]?.endsWith("validate.ts"))
  console.log(JSON.stringify(await validateGen123(true), null, 2));
