...
type GbaMap = {
  ...
  readonly connections?: readonly { readonly map: string; readonly direction: Direction }[];
  readonly warp_events?: readonly { readonly dest_map: string }[];
};
...
function routeCandidates(
  ...
  const regions = source === "firered" ? ["kanto"] : source === "emerald" ? ["hoenn"] : ["johto", "kanto"];
...
function resolveAuthority(
  ...
  const regionPrefixes = source === "firered" ? ["kanto"] : source === "emerald" ? ["hoenn"] : ["johto", "kanto"];
...
async function parseGba(
  ...
  for (const map of maps) {
    const slug = resolveAuthority(map.region_map_section, source, knownSlugs);
  ...
  for (const map of maps) {
    ...
    for (const connection of map.connections ?? []) {
      ...
      edges.push({ fromSlug, toSlug, connectionKey: connection.direction, source });
    }
...
function crystalMapConstant(name: string): string {
  return name
...
function parseCrystalMapAuthorities(text: string): Map<string, string> {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const parts = trimmed.slice(4).split(",").map((value) => value.trim());
    const name = parts[0];
    const landmark = parts[3];
    if (name !== undefined && landmark?.startsWith("LANDMARK_") === true) result.set(name, landmark);
  }
  return result;
}
...
async function parseCrystal(
  ...
  const mapByConstant = new Map(
    [...authorityByMap.keys()].map((name) => [crystalMapConstant(name), name] as const),
  );
  const locationByMap = new Map<string, string>();
  for (const [map, authority] of authorityByMap) {
    const slug = resolveAuthority(authority, "crystal", knownSlugs);
  ...
  for (const file of mapFiles) {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      const parts = trimmed.slice("warp_event ".length).split(",").map((value) => value.trim());
...
export async function loadGen123WorldTopology(
  ...
  const regionBySlug = new Map(
    locationRows.map((row) => [requiredText(row, "identifier"), requiredInt(row, "region_id")] as const),
  );
  ...
  return { locationSlugs: locations, edges: allEdges, sourceLocationCounts, sourceEdgeCounts };
}
