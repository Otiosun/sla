...
export class BattleOperationalReadService {
  ...
  public async forPlayer(battleId: string, playerId: string): Promise<Result<OperationalBattleView>> {
    return this.repository.read(async (transaction) => {
      ...
      if (side === undefined) return err(appError("ACTION_INVALID", "Player is not a participant in this battle"));
      const ruleset = await transaction.loadRuleset(root.rulesetId);
      if (ruleset === null) return err(appError("FEATURE_UNAVAILABLE", "Battle ruleset is unavailable"));
