...
function repository(overrides: Partial<AdminTeamRepository> = {}): AdminTeamRepository {
  return {
    ...
    addPrincipal: vi.fn(async (input) => ({
      principalId: input.playerId,
    ...
    setReceptionStaff: vi.fn(async (input) => ({
      principalId: input.targetPrincipalId,
      displayName: "Staff",
      status: "ACTIVE" as const,
      owner: false,
      receptionStaff: input.active,
      revision: "1",
      capabilities: ["central.view", "player.registration.read"],
    })),
