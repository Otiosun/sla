...
function setup() {
  ...
  const readFacade = {
    ...
    getPlayer: vi.fn(),
    getEconomyAnalytics,
  } as unknown as Parameters<typeof createAdminApiServer>[0]["readFacade"];
...
describe("F8.3 economy analytics HTTP composition", () => {
  it("exposes one bounded global read-only economy aggregate with trusted server context", async () => {
