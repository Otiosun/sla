import { afterEach, describe, expect, it, vi } from "vitest";
import { createAdminApiServer } from "../../src/adapters/admin-api/fastify-server.js";

const TOKEN = "x".repeat(64);
const ORIGIN = "https://admin-staging.example.com";
const PRINCIPAL_ID = "11111111-1111-4111-8111-111111111111";

const identity = {
  principalId: PRINCIPAL_ID,
  environment: "staging" as const,
  identityRef: "cloudflare-access:pokemon-rpg.cloudflareaccess.com:stable-subject",
  displayEmail: "admin@example.com",
  accessSession: {
    tokenFingerprint: "a".repeat(64),
    issuedAt: new Date("2026-09-01T10:00:00.000Z"),
    notBefore: new Date("2026-09-01T10:00:00.000Z"),
    expiresAt: new Date("2026-09-01T11:00:00.000Z"),
  },
};

const snapshot = {
  inbox: {
    counts: { RECEIVED: 0, PROCESSING: 1, PROCESSED: 10, FAILED: 1 },
    recent: [],
  },
  outbox: {
    counts: { PENDING: 2, SENDING: 0, SENT: 20, FAILED: 1, DEAD: 1 },
    recent: [],
    deadLetter: [],
  },
};

const servers: ReturnType<typeof createAdminApiServer>[] = [];
afterEach(async () => {
  await Promise.all(servers.splice(0).map((server) => server.close()));
});

function setup() {
  const getMessagingOperations = vi.fn(async () => snapshot);
  const prepareMutation = vi.fn();
  const consume = vi.fn().mockResolvedValue({ allowed: true, retryAfterSeconds: 60 });
  const dependencies = {
    allowedOrigin: ORIGIN,
    authenticator: { authenticate: vi.fn(async () => identity) },
    sessionGuard: { authorize: vi.fn(async () => identity) },
    sessionService: {
      getSession: vi.fn(async () => ({
        principalId: PRINCIPAL_ID,
        roles: ["SENIOR_ADMIN"],
        capabilities: [{ key: "messaging.operations.read", riskTier: 0 as const }],
        scopes: [{ scopeType: "GLOBAL" as const, scopeId: null }],
        environment: "staging" as const,
      })),
    },
    readFacade: {
      searchPlayers: vi.fn(async () => ({ items: [], nextCursor: null })),
      getPlayer: vi.fn(),
      getMessagingOperations,
    },
    mutationFacade: { prepareMutation },
    rateLimiter: { consume },
  };
  const server = createAdminApiServer(dependencies);
  servers.push(server);
  return { server, getMessagingOperations, prepareMutation, consume };
}

describe("Admin API messaging operations read", () => {
  it("exposes a strict authenticated metadata-only GET with its own rate-limit budget", async () => {
    const { server, getMessagingOperations, prepareMutation, consume } = setup();
    const response = await server.inject({
      method: "GET",
      url: "/admin/v1/messaging/operations",
      headers: { origin: ORIGIN, "cf-access-jwt-assertion": TOKEN },
    });

    expect(response.statusCode).toBe(200);
    expect(response.json()).toEqual(snapshot);
    expect(consume).toHaveBeenCalledWith({
      principalId: PRINCIPAL_ID,
      operation: "messaging.operations.read",
    });
    expect(getMessagingOperations).toHaveBeenCalledWith({
      principalId: PRINCIPAL_ID,
      environment: "staging",
      correlationId: response.headers["x-correlation-id"],
    });
    expect(prepareMutation).not.toHaveBeenCalled();
    expect(response.headers["cache-control"]).toBe("no-store");
  });

  it("rejects browser-selected environment, limits or queue selectors", async () => {
    const { server, getMessagingOperations } = setup();
    for (const query of ["environment=production", "limit=100", "queue=outbox"]) {
      const response = await server.inject({
        method: "GET",
        url: `/admin/v1/messaging/operations?${query}`,
        headers: { origin: ORIGIN, "cf-access-jwt-assertion": TOKEN },
      });
      expect(response.statusCode).toBe(400);
    }
    expect(getMessagingOperations).not.toHaveBeenCalled();
  });
});
