...
function setup() {
  const searchContent = vi.fn(async () => ({ items: [], nextCursor: null }));
  ...
  servers.push(server);
  return { server, searchContent, consume };
}

describe("Admin API content library boundary", () => {
  it("exposes a GET-only searchable content library with server-owned context", async () => {
    const { server, searchContent, consume } = setup();
    const response = await server.inject({
      method: "GET",
      url: "/admin/v1/content?query=Pikachu&resourceKind=SPECIES&releaseStatus=DRAFT&active=true&limit=25",
  ...
  it.each(["principalId", "environment", "roles", "capabilities", "scopes"] as const)(
    ...
    async (field) => {
      const { server, searchContent } = setup();
      const response = await server.inject({
        method: "GET",
        url: `/admin/v1/content?${field}=attacker-controlled`,
        headers: { origin: ORIGIN, "cf-access-jwt-assertion": TOKEN },
      });
