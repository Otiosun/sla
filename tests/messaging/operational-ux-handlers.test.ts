...
import { createOperationalUxRoutes, type OperationalUxDependencies } from "../../src/modules/messaging/operational-ux-handlers.js";
...
function dependencies(overrides: Record<string, unknown> = {}): OperationalUxDependencies {
  const base = {
    registration: {
      resolvePlayer: vi.fn(async () => ok({ playerId: PLAYER_ID, state: "COMPLETE", created: false })),
      resolveOrCreatePlayer: vi.fn(async () => ok({ playerId: PLAYER_ID, state: "COMPLETE", created: false })),
    ...
    starter: {
      ...
      prepareStarterSelection: vi.fn(async () => ok({ playerId: PLAYER_ID, starterClaimKey: "claim", options: [] })),
      grantStarter: vi.fn(),
      completeOnboarding: vi.fn(async () => ok({ playerId: PLAYER_ID, state: "COMPLETE" })),
      getProfile: vi.fn(async () => ok({
        playerId: PLAYER_ID,
        playerStatus: "ACTIVE",
        trainerName: "Red",
    ...
    encounter: {
      activeForPlayer: vi.fn(async () => ok({
    ...
    battle: {
      forPlayer: vi.fn(async () => ok({
        playerSideNo: 1,
        legalActions: [
          { type: "USE_MOVE", actorParticipantId: "own", moveSlot: 1, targetParticipantId: "wild" },
        ...
        state: {
          ...
          version: 9n,
          sides: [
            { sideNo: 1, controllerKind: "PLAYER", playerId: PLAYER_ID, activeParticipantId: "own", result: null },
            { sideNo: 2, controllerKind: "WILD", playerId: null, activeParticipantId: "wild", result: null },
          ],
          combatants: [
            {
              participantId: "own",
    ...
    reads: {
      ...
      listInventory: vi.fn(async () => [{ itemId: "item-potion", itemSlug: "potion", displayName: "Potion", quantity: 3n }]),
      listPokedex: vi.fn(async () => [{ speciesId: "species-charmander", nationalDex: 4, speciesSlug: "charmander", displayName: "Charmander", seenCount: 2n, caughtCount: 1n }]),
      activeBattleId: vi.fn(async () => "battle-1"),
      speciesDisplayName: vi.fn(async () => "Pidgey"),
      moveDisplayNames: vi.fn(async () => new Map([["move-tackle", "Tackle"], ["move-growl", "Growl"]])),
    },
  };
...
describe("Phase 13 operational WhatsApp UX", () => {
  ...
  it("renders profile, team, inventory and Pokedex as readable mobile text", async () => {
    ...
    expect(textOf(await app.dispatch(context("$equipe", "team")))).toContain("Charmander · Nv. 5 · HP 19");
    expect(textOf(await app.dispatch(context("$inventario", "inventory")))).toContain("Potion ×3");
    expect(textOf(await app.dispatch(context("$pokedex", "pokedex")))).toContain("#0004 Charmander · vistos 2 · capturados 1");
  });
  ...
  it("emits revision-bound travel commands and rejects stale text before calling the owner", async () => {
    ...
    expect(deps.world.travel).toHaveBeenCalledWith(expect.objectContaining({
      playerId: PLAYER_ID,
      destinationAreaId: "area-route-1",
      expectedRevision: 7n,
